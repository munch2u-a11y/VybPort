...
assert.equal(state.choose(choices).key, "local:3", "a fresh browser should favor the real terminal conversation");
values.set("vybport.garage.agent", "mcp:8");
assert.equal(state.choose(choices).key, "local:3", "a stale page-local choice must not hide the CLI transcript");
